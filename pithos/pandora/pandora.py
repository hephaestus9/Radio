...
class Pandora(object):
    ...
    def json_call(self, method, args={}, https=False, blowfish=True):
        ...
        #radiologger.log(url, "INFO")
        #radiologger.log(data, "INFO")
        ...
        try:
            ...
            self.radiologger.log("HTTP error: %s", (e), "ERROR")
            raise PandoraNetError(str(e))
        except urllib2.URLError as e:
            self.radiologger.log("Network error: %s", (e), "ERROR")
            if e.reason[0] == 'timed out':
                raise PandoraTimeout("Network error: %s", submsg="Timeout")
            else:
                raise PandoraNetError("Network error: %s", submsg=e.reason[1])

        #radiologger.log(text, "INFO")
        ...
        if tree['stat'] == 'fail':
            code = tree['code']
            msg = tree['message']
            self.radiologger.log('fault code: ' + str(code) + ' message: ' + msg, "ERROR")
    ...
    def connect(self, client, user, password):
        ...
        self.radiologger.log("Time offset is %s" % self.time_offset, "INFO")
    ...
    def get_stations(self, *ignore):
        ...
        self.stations = [Station(self, i, self.radiologger) for i in stations]
    ...
    def add_station_by_music_id(self, musicid):
        ...
        station = Station(self, d, self.radiologger)
    ...
    def add_feedback(self, trackToken, rating):
        self.radiologger.log("pandora: addFeedback", "INFO")
...
class Station(object):
    def __init__(self, pandora, d, radiologger):
    ...
    def transformIfShared(self):
        if not self.isCreator:
            self.radiologger.log("pandora: transforming station", "INFO")
            self.pandora.json_call('station.transformSharedStation', {'stationToken': self.idToken})
            self.isCreator = True

    def get_playlist(self):
        self.radiologger.log("pandora: Get Playlist", "INFO")
        ...
        for i in playlist['items']:
            if 'songName' in i:  # check for ads
                songs.append(Song(self.pandora, i, self.radiologger))
        return songs
    ...
    def rename(self, new_name):
        if new_name != self.name:
            ...
            self.radiologger.log("pandora: Renaming station", "INFO")
            self.pandora.json_call('station.renameStation', {'stationToken': self.idToken, 'stationName': new_name})
            self.name = new_name

    def delete(self):
        self.radiologger.log("pandora: Deleting Station", "INFO")
...
class Song(object):
    ...
    def audioUrl(self):
        ...
        try:
            ...
            self.radiologger.log("Using audio quality %s: %s %s" % (quality, q['bitrate'], q['encoding']), "INFO")
            return q['audioUrl']
        except KeyError:
            self.radiologger.log("Unable to use audio format %s. Using %s" %
                           (quality, self.audioUrlMap.keys()[0]), "WARNING")
